...
class CityscapesInstance2SemSegEvaluator(CityscapesEvaluator):
    ...
    def process(self, inputs, outputs):
        from cityscapesscripts.helpers.labels import name2label
        self.process_instance(inputs, outputs, name2label)
        self.process_instance2semantic(inputs, outputs, name2label)

    def process_instance(self, inputs, outputs, name2label):
        for input, output in zip(inputs, outputs):
            file_name = input["file_name"]
            basename = os.path.splitext(os.path.basename(file_name))[0]
            pred_txt = os.path.join(self._temp_dir, basename + "_pred.txt")
            ...
            if "instances" in output:
                ...
                with open(pred_txt, "w") as fout:
                    for i in range(num_instances):
                        ...
                        class_id = name2label[classes].id
                        score = output.scores[i]
                        #### cindy , this is for not uda maskrcnn
                        if classes == 'person' and  score < 0.9:
                            continue
                        elif classes == 'rider' and score < 0.8:
                            continue
                        elif classes == 'motorcycle' and score < 0.5:
                            continue
                        elif classes == 'bicycle' and score < 0.5:
                            continue
                        elif classes == 'car' and score < 0.5:
                            continue
                        elif classes == 'truck' and score < 0.9:
                            continue
                        elif classes == 'bus' and score < 0.9:
                            continue
                        elif classes == 'train' and score < 0.5:
                            continue
                        # mask = output.pred_masks[i].numpy().astype("uint8")
                        mask = output.pred_masks[i]
                        pred[mask] = class_id
                        fout.write(
    ...
    def evaluate(self):
        ...
        ret_instance = self.evaluate_instance()
